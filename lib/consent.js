...
// Forget every stored approval, so the next run of each script prompts again. Returns how many
// were dropped. Deliberately unguarded by a confirmation: the effect is to ask MORE often, and the
// cost of doing it by accident is one extra prompt.
function revokeAllApprovals() {
  const all = atom.config.get(CONFIG_KEY) || [];
  const count = all.length;
  // `unset` rather than `set([])` so the key returns to its schema default instead of persisting
  // an empty array in the user's config file.
  atom.config.unset(CONFIG_KEY);
  return count;
}

// Approvals currently on record, newest first — what a revoke would clear.
function listApprovals() {
  return [...(atom.config.get(CONFIG_KEY) || [])].sort((a, b) =>
    String(b?.approvedAt || "").localeCompare(String(a?.approvedAt || ""))
  );
}

module.exports = { ensureConsent, revokeAllApprovals, listApprovals };
