...
// { id, icon|glyph, title, action?, className? } — action(ctx) is called on click.
// `icon`, `title` and `className` may be functions, re-evaluated on each render so a
// control can reflect live state (e.g. a toggle that shows its current direction).
