...
function renderCluster(pane, cluster) {
  ...
  for (const def of defs) {
    ...
    // Prefer a named codicon (from tranquil-theme-icons' global font); fall back
    // to a literal Unicode glyph for callers/mockups that still use `glyph`.
    if (def.icon) {
      const glyph = document.createElement("span");
      glyph.className = "codicon codicon-" + def.icon;
