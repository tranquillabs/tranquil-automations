...
// Custom right-aligned tab-bar controls injected into every pane container's existing
// `.tab-bar` (center and all three docks) from this owned package. The tabs package is
...
// Every pane container. The bottom dock was originally left out — nothing registered controls
// there — but the Automation Runs panel is bottom-only, so leaving it out meant no cluster was
// ever injected into its tab-bar and its controls could not render at all. A container with
// nothing registered for its active item still shows no cluster, so including it costs nothing.
const LOCATIONS = ["center", "left", "right", "bottom"];
...
// { id, icon|glyph, title, action?, className?, visible? } — action(ctx) is called on click.
// `icon`, `title`, `className` and `visible` may be functions, re-evaluated on each
// render so a control can reflect live state (e.g. a toggle that shows its current
// direction, or hide entirely while some condition holds) without unregistering.
...
function renderCluster(pane, cluster) {
  const item = pane.getActiveItem();
  // visible may be a function, evaluated per-render like icon/title/className, so a
  // control can come and go with live state (e.g. tree-view's buttons while the
  // project is empty) without needing its own unregister/re-register.
  const defs = itemsForPane(pane).filter((def) =>
    def.visible === undefined
      ? true
  ...
  for (const def of defs) {
    ...
    // Optional extra classes, so a control can carry transient state a swapped icon can't express
    // — the Runs panel's Cancel goes green with a spinner while the kill escalates. Evaluated per
    // render like icon/title, and space-separated for more than one.
    const extraClass = typeof def.className === "function" ? def.className() : def.className;
    if (extraClass) btn.classList.add(...String(extraClass).split(/\s+/).filter(Boolean));
    btn.title = title || "";
...
function injectInto(tabBar, pane, disposables) {
  ...
  // Project-state-driven `visible` gates (e.g. tree-view's buttons while the
  // project is empty) don't fire any of the pane events above on their own.
  disposables.add(atom.project.onDidChangePaths(render));
  disposables.add(
    pane.onDidDestroy(() => {
...
// Sets up injection across every pane container. Returns a Disposable.
function activate() {
...
// Repaint every cluster. register()/dispose() already do this, but a consumer whose button
// LABEL or ICON depends on live state (tranquil-debug's continue/pause, whose step buttons are
// only enabled while paused) has no other way to ask for a re-render: icon and title may be
// functions, but they are only re-evaluated when something else triggers a render.
function refresh() {
  refreshAll();
}

module.exports = { activate, register, refresh };
