...
//
// Every layout read happens before the first style write: reading a layout value
// after a write forces the browser to recompute layout synchronously to answer it,
// and `currentWrap.offsetWidth` used to be read after `style.top` had already been
// set, so each call paid for two layouts instead of one.
function reposition() {
  ...
  // tvRect.right is the outer border-box edge, behind the vertical scrollbar
  // when the tree overflows — subtract the scrollbar width (0 when absent) so
  // the buttons don't overlap it.
  const scrollbarWidth = tv.offsetWidth - tv.clientWidth;
  const wrapWidth = currentWrap.offsetWidth;
  const top = Math.round(rowRect.top + rowRect.height / 2);
  const left = Math.round(tvRect.right - scrollbarWidth - wrapWidth - 4);

  currentWrap.style.top = top + "px";
  currentWrap.style.left = left + "px";
}

// Coalesced form for the high-frequency scroll/resize path — at most one
// reposition per frame. `readDocument` runs after that frame's writers, so the
// measurements above see settled layout.
...
// Capture-phase on `document`, so this fires for every scrollable container in
// the workspace — coalesce it rather than repositioning per event.
