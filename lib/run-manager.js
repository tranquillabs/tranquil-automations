...
class RunManager {
  ...
  //
  // Answered from `this.live` — the child processes we are actually holding — and not from the
  // record's `state`. A `state` of "running"/"paused" is a claim that can outlive its process:
  // a child that dies without its exit handler landing leaves the record in flight forever, and
  // anything gated on it (the Run button) stays hidden for that file for the life of the window
  // with no way back.
  //
  // `this.live` is emptied on child exit, on spawn error, and wholesale on dispose(). cancel()
  // reaches it indirectly — it kills the child, whose exit handler does the delete — with a
  // SIGKILL backstop, so it cannot hang there either.
  isDebugging(scriptPath) {
    for (const runId of this.live.keys()) {
      const record = this.runs.find((r) => r.runId === runId);
      if (record && record.mode === "debug" && record.scriptPath === scriptPath) return true;
    }
    return false;
  }

  // Drop finished runs from the history. In-flight ones (running / paused, including a debug
  // session at a breakpoint) are KEPT: their record is the only handle on a live child — it is
  // what Cancel acts on and where streaming output lands — so removing it would strand the
  // process with no way to reach it. Returns how many were dropped.
  clearRuns() {
    const before = this.runs.length;
    this.runs = this.runs.filter((r) => r.state === "running" || r.state === "paused");
    const cleared = before - this.runs.length;
    if (cleared) this.emitter.emit("did-update", null);
    return cleared;
  }
  ...
  async run({ scriptPath, sourceOverride, trigger = "manual", debug = false }) {
    ...
    // File access is declared or absent — there is no implicit grant for the script's own
    // directory. `read`/`write` used to mean "beyond your own folder", with scriptDir added
    // silently, which meant a header saying `none` still permitted writing next to the script.
    // Relative paths resolve against the script's directory, so a script names what it touches
    // (`write=output.md`, `write=results`); a bare "." is rejected in permissions.js because
    // Deno's path grants are recursive and it would restore the blanket.
    const resolvePaths = (paths) =>
      paths.map((p) => (path.isAbsolute(p) ? p : path.resolve(scriptDir, p)));

    // The two read paths Deno needs to LOAD the program, as opposed to anything the script reads:
    // the entry module (main.ts reaches it through a dynamic import, which is permission-checked)
    // and the SDK directory the entry statically imports `tranquil/automation` from. Verified
    // both are required — with only the entry, resolving the SDK import fails.
    //
    // Deliberately file- and package-scoped, never the script's directory: with just these,
    // reading or writing a sibling data file is still denied, so `files.read("input.txt")` needs
    // an explicit `read=input.txt`. These paths are the app's own code, not the user's data.
    const machineryRead = [entry, path.dirname(bootstrapEntry())];
    args.push(`--allow-read=${[...machineryRead, ...resolvePaths(g.read)].join(",")}`);
    // Nothing in the machinery writes, so write is purely what the header asked for; an empty
    // list omits the flag, which is Deno's default-deny.
    if (g.write.length) args.push(`--allow-write=${resolvePaths(g.write).join(",")}`);
    ...
    const record = {
      ...
      mode,
      // A selection run looks identical to a whole-file run everywhere it is reported, which is
      // how a stray selection — a double-click leaves one — silently runs two words instead of
      // the file, then fails with a ReferenceError pointing at a temp file named after a UUID.
      // Recording it lets the runs panel say so.
      selection: sourceOverride != null ? String(sourceOverride).trim().split("\n").length : 0,
      inspectorPort,
      scriptPath,
      trigger,
      state: "running",
    ...
    // A child that fails to spawn at all (bad deno path, ENOENT) emits "error" and NEVER "exit",
    // so without settling the record here it stays "running" for the life of the window: a row
    // whose Cancel can do nothing, and — before Run stopped depending on it — a debug run that
    // kept `isDebugging()` true forever. Settle it as failed; the exit handler is a no-op if it
    // does also fire, since the record is no longer in flight.
    child.on("error", (err) => {
    ...
    child.on("exit", (code) => {
      ...
      if (record.state === "failed") {
        ...
        // A selection that references something the file declares elsewhere fails with a bare
        // "x is not defined" against a UUID-named temp file. Say which of the two ran, and why
        // the name is missing, rather than leaving that to be inferred.
        const missing = record.selection && /(\w+) is not defined/.exec(record.output);
        const hint = record.selection
          ? `\n\nRan a selection (${record.selection} line${record.selection === 1 ? "" : "s"}), ` +
            `not the whole file.` +
            (missing
              ? ` A selection inherits the file's imports but not its variables, and "${missing[1]}" ` +
                `is defined elsewhere in the file — include its declaration, or clear the selection.`
              : ` Clear the selection to run the file.`)
          : "";
        notify("addError", `Automation failed: ${path.basename(scriptPath)}`, {
          detail: firstErrorLine.trim() + hint,
