...
class AutomationRunsView {
  ...
  render() {
    ...
    if (selected) {
      ...
      if (selected.state === "running" || selected.state === "paused") {
        const cancel = document.createElement("button");
        cancel.classList.add("btn", "btn-sm", "run-cancel");
        // Cancelling is not instant — the runner gives the script a CANCEL frame, then escalates
        // to SIGTERM and SIGKILL over ~3s — so the button reports progress the way Copy does,
        // rather than looking inert while the escalation runs. The flag lives on the view, not on
        // the element: output keeps streaming during cancellation and every chunk rebuilds these
        // buttons, so per-element state would be wiped on the next render.
        const cancelling = this.cancellingRunId === selected.runId;
        cancel.textContent = cancelling ? "Cancelling" : "Cancel";
        cancel.classList.toggle("cancelling", cancelling);
        // Not `disabled`: the theme greys disabled buttons out, which would fight the green.
        // The handler below guards against a second press, and CSS drops pointer events.
        cancel.addEventListener("mousedown", (e) => {
          if (e.button !== 0) return;
          e.preventDefault();
          if (this.cancellingRunId === selected.runId) return;
          this.cancellingRunId = selected.runId;
          AutomationRunsView.runManager?.cancel(selected.runId);
          this.render();
        });
        actions.appendChild(cancel);
      } else if (this.cancellingRunId === selected.runId) {
