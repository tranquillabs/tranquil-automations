...
const { openMockups, watchTheme, registerMockupControls } = require("./mockups.js");
...
export default {
  ...
  activate(state) {
    ...
    // Business-theme reference mockups + custom tab-bar controls, gated behind a
    // config flag. Deferred to the next tick so the workspace (and the tabs
    // package) has finished its own startup deserialization first.
    if (atom.config.get("tranquil-automations.showBusinessMockups")) {
      this.subscriptions.add(paneControls.activate());
      // Example pane-control registrations for the two mockups.
      registerMockupControls(this.subscriptions);
  ...
  runInWebview() {
    ...
    if (!editor) return atom.notifications.addWarning("No active editor");
    const filePath = editor.getPath();
    if (!filePath || !filePath.endsWith(".js")) {
      return atom.notifications.addWarning("Active file is not a .js automation");
    }
  ...
  async runScript(content, scriptPath) {
    ...
    try {
      ...
      return atom.notifications.addError("Cannot connect to browser", {
    ...
    const tranquil = {
      ...
      autoInject: {
        register: (urlPattern) => {
          if (!scriptPath) {
            atom.notifications.addWarning("Cannot register unsaved script for auto-inject");
            return;
          }
          ...
          atom.notifications.addSuccess(`Auto-inject registered for /${patternStr}/`);
        },
        unregister: () => {
          ...
          atom.notifications.addInfo("Auto-inject unregistered");
        },
      },
    };
    ...
    try {
      ...
      if (e.userFacing) {
        ...
        atom.notifications.addError(`Script error in ${label}`, { detail: e.message });
      }
    }
  },
  ...
  registerCurrentFile() {
    ...
    const filePath = editor.getPath();
    if (!filePath || !filePath.endsWith(".js")) {
      return atom.notifications.addWarning("Active file is not a .js file");
    }

    const saved = atom.config.get("tranquil-automations.registeredAutomations") || [];
    if (saved.some((r) => r.path === filePath)) {
      const existing = saved.find((r) => r.path === filePath);
      return atom.notifications.addInfo(`Already registered: Automations: ${existing.name}`);
    }
    ...
    atom.notifications.addSuccess(`Registered: Automations: ${name}`);
  },
  ...
  unregisterCurrentFile() {
    ...
    if (!editor) return atom.notifications.addWarning("No active editor");
    const filePath = editor.getPath();
    if (!filePath) return atom.notifications.addWarning("File has no path");

    const saved = atom.config.get("tranquil-automations.registeredAutomations") || [];
    const entry = saved.find((r) => r.path === filePath);
    if (!entry) return atom.notifications.addWarning("File is not registered");
    ...
    atom.notifications.addInfo(`Unregistered: Automations: ${entry.name}`);
  },
