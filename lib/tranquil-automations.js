...
const runsPanelControls = require("./runs-panel-controls.js");
...
export default {
  ...
  activate(state) {
    ...
    this.subscriptions.add(
      atom.commands.add("atom-workspace", {
        "tranquil-automations:toggle-runs-panel": {
          ...
          // Toggles the PANEL, not the dock. Closing used to call container.hide(), which took the
          // whole bottom dock with it — hiding the Terminal and anything else sharing it, and
          // leaving the runs item still there so the dock came back with it next time. Destroying
          // the item removes just this panel; the dock stays open for its other tabs, and empties
          // itself only when nothing is left, which is Atom's own behaviour for an empty dock.
          //
          // Present but hidden (the user closed the dock with Cmd-J) counts as "not showing", so
          // the key reveals it rather than destroying something that was never visible.
          didDispatch: () => {
            ...
            if (existing && showing) {
              ...
              if (pane) pane.destroyItem(existing);
    ...
    this.subscriptions.add(
      atom.commands.add("atom-workspace", {
        ...
        "tranquil-automations:revoke-all-permissions": {
          displayName: "Automations: Revoke All Permissions",
          didDispatch: () => this.revokeAllPermissions(),
        },
      })
    );
    ...
    this.subscriptions.add(runsPanelControls.activate());
    this.subscriptions.add(
    ...
    // Bring the Run button back when a debug run ends. Its matcher calls isDebugging(), but
    // pane-controls only re-renders on its own triggers (active item, registry change), and the
    // moment isDebugging() flips is none of them: ending a session tears down tranquil-debug's
    // transport row and refreshes SYNCHRONOUSLY, while the Deno child only exits afterwards —
    // closing the inspector socket is what lets it exit at all. So that refresh still sees a live
    // debug child, and by the time the child is actually gone nothing is left to trigger a render.
    // Without this, Run stays hidden until some unrelated render happens to run.
    //
    // Filtered to debug runs that have left flight, so the output chunks of a chatty run — which
    // emit did-update on every line while still "running" — do not repaint every cluster.
    this.subscriptions.add(
      this.runManager.onDidUpdate((record) => {
        if (
          record &&
  ...
  // The runner reads the script from DISK (run-manager reads scriptPath), so a modified buffer
  // would run the previous version — silently, and looking exactly like an edit that changed
  // nothing. An empty never-written file is the loud version of the same bug: it gets refused for
  // declaring no permissions, naming a file the author thought they had just written. Saving is
  // the least surprising fix; refusing would put a Cmd-S in the middle of every REPL iteration.
  // Returns false only when the save itself failed.
  async _saveIfModified(filePath) {
    ...
    try {
      ...
      notify("addWarning", "Could not save the automation", { detail: e.message });
      return false;
    }
  },

  async runAutomation(editor) {
    ...
    if (!(await this._saveIfModified(filePath))) return;
    ...
    const disposable = atom.commands.add("atom-workspace", {
      [key]: {
        ...
        didDispatch: async () => {
          ...
          // Registered commands run a path, not an editor — but that file may still be open and
          // modified in this window, and running the stale disk copy is the same trap.
          if (!(await this._saveIfModified(filePath))) return;
  ...
  // Forget every stored consent approval, so each script asks again on its next run. There is no
  // confirmation step: revoking only ever makes the app ask MORE, and the cost of a misfire is one
  // extra prompt. The toast names the count so it is obvious whether anything was actually held.
  revokeAllPermissions() {
    const revoked = revokeAllApprovals();
    if (revoked === 0) {
      return notify("addInfo", "No stored permissions to revoke");
    }
