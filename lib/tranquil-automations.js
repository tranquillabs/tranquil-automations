...
export default {
  ...
  // file path (TreeView#selectActiveFile → deselect()). Because Tranquil uses
  // pathless tabs heavily (browser tabs), switching to one wipes the tree
  // highlight — and single-clicking a file appears to need two clicks, since the
  // pending open leaves a browser tab active and the resulting deselect erases
  // the click's own selection. tree-view is third-party, so we patch the live
  // instance: when there's no active project file, leave the current selection
  // untouched instead of deselecting. Returns true once the instance is patched
  // (or already was), false if tree-view isn't active yet.
  patchTreeViewKeepSelection() {
    const treeView = atom.packages.getActivePackage("tree-view")?.mainModule
      ?.treeView;
    if (!treeView || treeView.__tranquilKeepSelection) return !!treeView;

    const self = this;
    const original = treeView.selectActiveFile.bind(treeView);
    treeView.selectActiveFile = function () {
      // A browser tab restored/reopened without its source .url reports no path,
      // so the tree can't follow it. Reverse-map the tab's URL back to its .url
      // bookmark and restore the link, so getActivePath() (and revealActiveFile,
      // which tree-view calls next) can highlight/expand it. tranquil-browser now
      // persists filePath, so this only backfills legacy/pre-fix tabs.
      const item = atom.workspace.getCenter().getActivePaneItem();
      if (item && item.getURL && !item.getPath?.()) {
        const mapped = self.resolveUrlFile(item.getURL());
      ...
      // selection rather than clearing it. If the file exists but isn't
      // rendered yet (collapsed parent), autoReveal handles expand + select.
    ...
    this.subscriptions.add(
      new Disposable(() => {
        if (treeView.__tranquilKeepSelection) {
          treeView.selectActiveFile = original;
          delete treeView.__tranquilKeepSelection;
        }
      })
    );
    return true;
  ...
  // Map an http(s) URL back to the project `.url` bookmark file that points at
  // it, by matching each file's `URL=` line (same match the browser's .url
  // opener uses). The index is cached for a few seconds so tab switching doesn't
  // re-read the tree on every activation.
  resolveUrlFile(url) {
    if (!url || url === "tranquil-browser://blank") return null;
    const key = String(url).replace(/\/$/, "");
    const now = Date.now();
    if (!this._urlIndex || now - (this._urlIndexAt || 0) > 5000) {
      this._urlIndex = this.buildUrlIndex();
      this._urlIndexAt = now;
    }
    return this._urlIndex.get(key) || null;
  },
  ...
  activate(state) {
    ...
    this.subscriptions.add(
      atom.commands.add("atom-workspace", {
        ...
        "tranquil-automations:toggle-tree-view-sync": {
          displayName: "Tree View: Sync Selection With Active Tab",
          didDispatch: () => {
            const next = !atom.config.get("tree-view.autoReveal");
            atom.config.set("tree-view.autoReveal", next);
            notify("addInfo", `Tree view sync ${next ? "on" : "off"}`);
          },
        },
        // Cycle the CENTER pane's tabs regardless of focus. Core
        // pane:show-next-item targets the active pane, so ctrl-tab does nothing
        // when a dock (e.g. the tree-view) is focused; these always act on the
        // center. Bound to ctrl-tab / ctrl-shift-tab in the client keymaps.
        "tranquil-automations:cycle-next-center-tab": {
          displayName: "Tabs: Next Tab (center pane)",
          didDispatch: () =>
            atom.workspace.getCenter().getActivePane()?.activateNextItem(),
        },
        "tranquil-automations:cycle-previous-center-tab": {
          displayName: "Tabs: Previous Tab (center pane)",
          didDispatch: () =>
            atom.workspace.getCenter().getActivePane()?.activatePreviousItem(),
        },
      })
    );

    // Keep the tree-view selection in sync with the active tab even when the
    // active pane item is pathless (e.g. a browser tab). tree-view is patched on
    // its instance; if it hasn't activated yet, retry when it does.
    if (!this.patchTreeViewKeepSelection()) {
      this.subscriptions.add(
        atom.packages.onDidActivatePackage((pkg) => {
          if (pkg.name === "tree-view") this.patchTreeViewKeepSelection();
        })
      );
    }
