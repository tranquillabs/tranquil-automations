...
export default {
  ...
  provideAutomationRunner() {
    ...
    return {
      // Deno executes what is on disk, so a dirty buffer puts breakpoints on lines the debugger
      // never sees — save before starting a debug run. Deliberately not saved for you: an editor
      // that writes your file because you pressed Run is doing something you did not ask for.
      runDebug: ({ scriptPath }) => rm.run({ scriptPath, trigger: "debug", debug: true }),
