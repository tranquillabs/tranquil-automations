...
class VerticalTabsView {
  constructor(state) {
    ...
    // Items added within the last moment → the timestamp they were added. Their
    // rows show a highlight that fades out (see `.just-added`). We store the
    // timestamp (not just membership) so render() can set a negative
    // animation-delay and RESUME the fade at the right point across the
    // re-renders a loading tab triggers, instead of restarting it each time.
    this.recentlyAdded = new Map();

    // Render coalescing. A single loading browser tab emits title-changed more than
    // once, and every item in the pane is subscribed, so bursts are the norm rather
    // than the exception — see scheduleRender().
    this.pendingRender = null;
    this.revealLastOnRender = false;
  ...
  watchPane(pane) {
    ...
    // appended at the bottom. Re-render, reveal it (scrollToBottom) so it isn't
    // off-screen below the fold, and briefly highlight it (see `.just-added` in
    // the stylesheet) — with many tabs open a new one is otherwise easy to miss.
    this.paneSubs.add(
      pane.onDidAddItem(({ item, moved }) => {
        // Highlight a genuinely new tab (not a drag/reorder), fading it out over
        // ~1s. Record when it was added; drop it once the fade is done.
        if (!moved && item !== this) {
          this.recentlyAdded.set(item, Date.now());
          setTimeout(() => {
            this.recentlyAdded.delete(item);
            this.scheduleRender();
          }, 1000);
        }
        // The reveal has to happen after the row exists, so it rides along with the
        // render rather than running before it.
        this.scheduleRender({ revealLast: true });
      })
    );
  ...
  // Coalesce renders to one per frame. Every item in the pane is subscribed to
  // title/modified/icon changes, and a loading browser tab emits title-changed more
  // than once — so a couple of tabs loading together used to mean repeated full list
  // rebuilds (plus a forceRepaint transform toggle each time) for a list the screen
  // can only draw once per frame anyway.
  scheduleRender({ revealLast = false } = {}) {
    if (revealLast) this.revealLastOnRender = true;
  ...
  render() {
    const revealLast = this.revealLastOnRender;
    this.revealLastOnRender = false;
    ...
    for (const item of pane.getItems()) {
      ...
      // Fading highlight on a just-opened tab so it's noticeable in a long list.
      // The negative animation-delay resumes the CSS fade at the right point, so
      // the re-renders a loading tab triggers don't restart it from full opacity.
      const addedAt = this.recentlyAdded.get(item);
      if (addedAt !== undefined) {
        row.classList.add("just-added");
        row.style.animationDelay = `-${Date.now() - addedAt}ms`;
      }
      ...
      // Coalesced: this fires per item, and a browser tab loading a page emits
      // title-changed more than once, so several tabs loading at once would
      // otherwise rebuild the whole list many times over within a single frame.
      const rerender = () => this.scheduleRender();
    ...
    if (revealLast) this.scrollToBottom();
  }
...
VerticalTabsView.setFileIconService = function (service) {
  ...
  for (const view of VerticalTabsView.instances) view.scheduleRender();
};
